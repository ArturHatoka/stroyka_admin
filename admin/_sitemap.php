...
//новости
$search = mysql_query("SELECT ".MySQLprefix."_urls.url FROM ".MySQLprefix."_news, ".MySQLprefix."_urls WHERE ".MySQLprefix."_news.id=".MySQLprefix."_urls.target_id AND ".MySQLprefix."_urls.target_type='news'");
if($search)
	if(mysql_num_rows($search)>0)
		while($srch = mysql_fetch_assoc($search)){
			$xmlsitemap .= '
