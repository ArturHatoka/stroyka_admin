		<div class="wide">
			...
			<div class="r-740<?php if($additional[29]==1){ ?> w-1024<?php } ?>">
				...
				<div class="pagetext">
					...
					<?php
					$p = $_GET['p']>0?$_GET['p']:1;
					$result = mysql_query("SELECT ".MySQLprefix."_news.pic, ".MySQLprefix."_news.h1, ".MySQLprefix."_news.date_create, ".MySQLprefix."_news.comment, ".MySQLprefix."_urls.url FROM ".MySQLprefix."_news, ".MySQLprefix."_urls WHERE ".MySQLprefix."_news.id=".MySQLprefix."_urls.target_id AND ".MySQLprefix."_urls.target_type='news' ORDER BY ".MySQLprefix."_news.id DESC LIMIT ".(20*($p-1)).", 20");
					$all_p = ceil(mysql_result(mysql_query("SELECT count(*) FROM ".MySQLprefix."_news"),0) / 20);
					if($result)
