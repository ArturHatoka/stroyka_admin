		<div class="wide">
			<div class="r-740<?php if($additional[29]==1){ ?> w-1024<?php } ?>">
				<div class="tree">
					<a href="/">Главная</a>/<a href="/<?=$katalog_a['url']?>/"><?=$katalog_a['menu']?></a><?php
					$tree = '';
					$tree_cat_ar[] = $seo['id'];
				...
				<div class="pagetext">
					...
					<b class="opis">Описание проекта:</b>
					...
					<?php } ?>
					<h2 class="pad-top-clear"><a>Другие подобные проекты</a></h2>
							...
							if(is_array($pics) && count($pics)>0)
								foreach($pics AS $picc)
									if(strlen($picc)>0 && $pic == 'admin/uploads/nophoto.png')
										$pic = $picc;
							?>
							<a class="good-a" href="/goods/<?=$goodi['id']?>/"><img src="/<?=str_replace(".","_small.",$pic)?>" /></a>
							<a href="/goods/<?=$goodi['id']?>/"><span><?=$goodi['name']?></span></a>
